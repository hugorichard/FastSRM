...
\newcommand{\bt}[1]{\todo[color=orange, size=\tiny]{BT: #1}}
...
of subjects or runs is large,\bt{True, but we the experiments don't make it so explicit ?} fitting the model requires a large amount of memory
...
%
There is a solid tradition of multivariate analysis of neuroimaging
data, that makes it possible to assess the information carried by the
data even under complex stimulation frameworks, eg movie watching,
story or music listening \bt{to be continued}.
%
%
What makes the analysis of functional magnetic resonance imaging (fMRI)
\bt{Question of scope: do want to target only fMRI or only MEEG ? Or only mention MEEG as possible extension in the discussion ?} data
particularly difficult is that the number of features (voxels) is usually much
larger than the number of available samples (time frames).
%
In such cases, dimension reduction methods are needed to
reduce the computational burden of subsequent data analysis.
%
The shared response
model~\cite{chen2015reduced} provides a principled way to perform dimension
reduction of brain imaging data from subjects performing the same task and is
therefore a natural basis for common source
...
or transfer learning~\cite{zhang2018transfer}.
%
...
%
Another issue of SRM is its non-identifiability (see the proof
...
\begin{figure}
  \centering
  \includegraphics[scale=0.3]{figures/srm/conceptual_figure31.png}\bt{in the figure, can you reaplce ``timeframe'' with ``time frame'' ?}
...
Let us consider $\xb_i = U_{\xb_i} \zb_i$ \bt{But any fill-rank orthogonal basis would work equally well (i.e. any orthogonal transform of the PCA basis)} a PCA of $\xb_i$ with the maximum
...
We call probSRM($\psi$)\bt{I don't unserstand what $\psi$ represents}
the probabilistic SRM algorithm modified such that updates
\begin{align}
...
%
We used nilearn~\cite{abraham2014machine} for fMRI data preprocessing,
Brainiak~\cite{kumar2020brainiak2} for the non-identifiable version of SRM that
implements the work of~\cite{chen2015reduced} and~\cite{anderson2016enabling}, Numpy~\cite{harris2020array} for
array processing, Scipy~\cite{2020SciPy-NMeth} its implementation of the
Hungarian algorithm, Matplotlib~\cite{hunter2007matplotlib} for plotting and
Sklearn~\cite{pedregosa2011scikit} for machine learning pipelines.
\bt{Please provide versions of the software used}
...
%
The parameters $\sigma_i$, $A_i$ and $\Sigma_s$ are generated randomly.
%
We sample the value of the subject specific noise level from a normal
distribution: $\sigma_i \sim \Ncal(0, 0.1)$ \bt{$\sigma_i$ has to be non-negative}.
%
...
of 395 timeframes and one block of 396 timeframes for the needs of our
experiments.
%
...
correct parameters can be recovered without ambiguity.\bt{I find this a bit ambiguous: you give te impression that you gurantee that the optimal solution is used. Rephrase.}
%
...
Time-segment matching experiment is first introduced
...
sense to assume that different subjects watching the movie would still have
closer representation of the same scenes than of different scenes.
%
...
subjects but one.
%
We select a target time-segment (9 consecutive timeframes) in the shared
...
%
% 
The mean accuracy is reported in Figure~\ref{fig:srm:timesegment} (bottom).
%
When the
...
efficient that implementations not using the optimal atlas.
...
mixing matrices.
%
Our optimal atlas provide the intuition that SRM-like methods do not need the
