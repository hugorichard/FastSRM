...
What makes the analysis of functional magnetic resonance imaging (fMRI) data
particularly difficult is that the number of features (voxels) is usually much
larger than the number of available samples (timeframes).
%
In such cases, dimension reduction methods are precious tools that allow to
...
on synthetic as well as on real data, it is much faster and more memory
...
and
\begin{align}
  c_2(\xb) = c_1 \exp(-\frac12( \sum_i
  \frac1{\sigma_i^2}\|\xb_i\|^2 - \dotp{  \EE[\sbb }{ \xb], \VV[\sbb | \xb]^{-1}
  \EE[\sbb | \xb] }))
\end{align}
are independent of $\sbb$.
%
Therefore,
\begin{align}
...
multiple times such as when cross validated results are needed.
%
In these cases,
...
$1000$.
%
It remains to show how to draw a correspondence between FastSRM and SRM which is
...
the full data.
%
Note than if memory allows it, these matrix products can be
...
%
In this work, we do not claim that FastSRM is the best method. We only see that
SRM is widely used and aim to provide a faster, more memory efficient and
identifiable implementation.  In our view FastSRM should be seen as a plugin
replacement for SRM.
...
not fully converge.
%
This means that in practice a much larger number of
...
correct parameters can be recovered without ambiguity.
%
...
\begin{minipage}{0.38\linewidth}
      \includegraphics[width=\textwidth]{figures/srm/identifiability.pdf}
    \end{minipage}
    \hfill
