...
from fastsrm.utils import (generate_data, apply_aggregate, apply_input_format,
                           to_path)
...
def generate_ica_friendly_data(n_voxels,
                               n_timeframes,
                               n_subjects,
                               datadir,
                               noise_level=0.1,
    ...
    S = np.random.multivariate_normal(np.zeros(3),
                                      np.diag([5, 2, 1]),
                                      size=(int(np.sum(n_timeframes)))).T
...
def test_voxelcentered():
    ...
    with pytest.raises(ValueError,
                       match=("Input data should be voxel-centered when "
                              "identifiability = ica")):
        srm.fit(X)
...
def test_fastsrm_class(identifiability):
    ...
    with tempfile.TemporaryDirectory() as datadir:
        ...
        srm = IdentifiableFastSRM(identifiability=identifiability,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("list_of_list", False, False, np.arange(
            1, n_voxels + 1), 1, [25, 24], None),
...
def test_recover_decorr_basis():
    X, W, S = generate_ica_friendly_data(n_voxels, n_timeframes, n_subjects,
                                         None, 0, "list_of_list")
    srm = IdentifiableFastSRM(n_components=3, identifiability="decorr")
    srm.fit(X)
